...
if pages == "Childhood Imms - Heatmap":
    ...
    if st.session_state.data is not None:
        ...
        if selected_age < 1:
            with st.expander(
                ":material/kitchen: Immunisation Schedule: **8 - 16 weeks**"
            ):
            ...
            with st.expander(
                ":material/kitchen: Immunisation Schedule: **70 - 79 years**"
            ):
        ...
        show_dataframe_toggle = st.sidebar.checkbox(
            "Show DataFrame",
            help="Show the corresponding DataFrame, allowing you to download a csv of the data.",
        )

        if show_dataframe_toggle:
            st.divider()
            st.markdown(
                "**DataFrame of Selected age**. - use the download `.csv` button at the top right to export this list to SMS patients."
            )
    ...
    if selected_vaccination_groups:
        ...
        for group in selected_vaccination_groups:
            if group == "-NO-VACCINE":
                ...
                st.write(
                    "-NO-VACCINE = 1 (List all Patients who has no vaccination record.)"
                )
            else:
                ...
                doses = st.sidebar.selectbox(
                    ...
                    options=list(
                        range(0, max_no + 1)
                    ),  # Ensure options are correctly defined
                    index=0,  # Default to 0 doses
                    key=group,  # Unique key for each selectbox to avoid issues with Streamlit's state
                )
    ...
    filtered_df = base_df[
        (base_df["age_years"] >= age_range[0]) & (base_df["age_years"] <= age_range[1])
    ]
    ...
    location = st.sidebar.selectbox(
        ...
        options=make_dropdown_list(influenza_df),
        index=0,
    )
    ...
    st.markdown(
        "Download the ImmunizeMe SystmOne Search, and import with default settings to SystmOne."
    )
    ...
    st.markdown(
        "Within **Clinical Reporting** navigate to where the search is located: **My Reports/Python-data/ImmunizeMe - RUN ME**"
    )
    st.markdown(
        "Right Click ImmunizeMe - RUN ME and **breakdown** results. Export the result to `.csv`"
    )
    c1, c2 = st.columns(2)
    with c1:
